...
def _build_playwright_cookie(
    ...
    return {
        ...
        'secure': bool(secure),
        'sameSite': _normalise_same_site(same_site) if same_site not in _VALID_SAME_SITE else same_site,
    }
...
def _parse_netscape_cookies(text: str) -> list[dict]:
    ...
    for lineno, line in enumerate(text.splitlines(), start=1):
        ...
        if len(parts) < 7:
            _log.warning(
                'Line %d: expected 7 tab-separated fields, got %d — skipping: %r',
        ...
        cookie = _build_playwright_cookie(
            ...
            path=path or '/',
            expires=expiry,
            http_only=False,   # Netscape format has no httpOnly column
            secure=secure,
            same_site='Lax',   # Netscape format has no sameSite column
        )
