...
powershell -NoProfile -Command "$ws = New-Object -ComObject WScript.Shell; $d = [System.IO.Path]::Combine([System.Environment]::GetFolderPath('Desktop'), 'Sin''X Automation.lnk'); $s = $ws.CreateShortcut($d); $s.TargetPath = '%INSTALL_DIR%\SinX-Launcher.exe'; $s.WorkingDirectory = '%INSTALL_DIR%'; if (Test-Path '%INSTALL_DIR%\icon.ico') { $s.IconLocation = '%INSTALL_DIR%\icon.ico,0' }; $s.Save()"

echo [4/4] Membuat Shortcut Start Menu...
powershell -NoProfile -Command "$ws = New-Object -ComObject WScript.Shell; $sm = [System.IO.Path]::Combine([System.Environment]::GetFolderPath('Programs'), 'Sin''X Automation.lnk'); $s = $ws.CreateShortcut($sm); $s.TargetPath = '%INSTALL_DIR%\SinX-Launcher.exe'; $s.WorkingDirectory = '%INSTALL_DIR%'; if (Test-Path '%INSTALL_DIR%\icon.ico') { $s.IconLocation = '%INSTALL_DIR%\icon.ico,0' }; $s.Save()"
...
if /i "%RUN_NOW%" neq "n" (
    start "" "%INSTALL_DIR%\SinX-Launcher.exe"
)
exit /b
